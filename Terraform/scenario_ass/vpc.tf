...
resource "aws_subnet" "public" {
  ...
  depends_on = [aws_internet_gateway.techcorp-gw]
}
...
resource "aws_internet_gateway" "techcorp-gw" {
  vpc_id = aws_vpc.techcorp-vpc.id

  tags = var.tags
}

resource "aws_eip" "techcorp-eip" {
...
resource "aws_nat_gateway" "techcorp-nat-gw" {
  for_each      = aws_subnet.public
  allocation_id = aws_eip.techcorp-eip[each.key].id
...
resource "aws_route_table" "public" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.techcorp-gw.id
  }
...
resource "aws_route" "private_nat" {
  for_each               = aws_subnet.private
  route_table_id         = aws_route_table.private[each.key].id
  nat_gateway_id         = aws_nat_gateway.techcorp-nat-gw[replace(each.key, "private", "public")].id
